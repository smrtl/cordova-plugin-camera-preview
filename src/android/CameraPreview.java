  ...
  public boolean execute(String action, JSONArray args, CallbackContext callbackContext) throws JSONException {
    ...
    if (setOnPictureTakenHandlerAction.equals(action)){
      ...
      if (PermissionHelper.hasPermission(permission)) {
        ...
        execArgs = args;
        PermissionHelper.requestPermission(this, permissionsReqId, permission);
      }
    }
    ...
    cordova.getActivity().runOnUiThread(new Runnable() {
      ...
      public void run() {
        ...
        try {
          ...
          if (containerView.getParent() != webView.getParent()) {
						if (containerView.getParent() != null) {
							((ViewGroup) containerView.getParent()).removeView(containerView);
						}
						FrameLayout.LayoutParams containerLayoutParams = new FrameLayout.LayoutParams(FrameLayout.LayoutParams.MATCH_PARENT, FrameLayout.LayoutParams.MATCH_PARENT);
						((ViewGroup) webView.getParent()).addView(containerView, containerLayoutParams);
					}
          //display camera bellow the webview
          if(toBack){
            webView.setBackgroundColor(0x00000000);
            ((ViewGroup)webView).bringToFront();
          }
          else{
