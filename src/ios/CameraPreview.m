        ...
        if (error) {
            ...
            switch ([[UIApplication sharedApplication] statusBarOrientation]) {
                ...
                case UIDeviceOrientationLandscapeRight:
                    imageToFilter = [imageToFilter imageByApplyingTransform:CGAffineTransformMakeRotation(M_PI)];
                    break;
                case UIDeviceOrientationPortrait:
                default:
                    imageToFilter = [imageToFilter imageByApplyingTransform:CGAffineTransformMakeRotation(M_PI/-2)];
            }
            
            // Apply color filter if any
            ...
            {
                NSLog(@"Failed to write image path %@", imagePath);
            }
            else
            {
                NSLog(@"Picture file saved: %@",imagePath);
                
                // Exclude from iCloud
                NSError *error = nil;
                NSURL *URL = [NSURL fileURLWithPath:imagePath];
                NSLog(@"Excluding from iCloud backup: %@", URL);
